...
C_parameter = open("C_parameter", "w")
...
'''
for C_val in [100, 300, 600, 1000, 1500, 2000]:
	print("C parameter value: {0: d}\n".format(C_val))
	print("training")
	svr = SVR(kernel='linear', C=C_val, max_iter=1000)
	svr.fit(dataset_X[0 : train_size], dataset_Y[0 : train_size])
	print("predicting")
	y_train_predict = svr.predict(dataset_X[0 : train_size])
	y_test_predict = svr.predict(dataset_X[train_size : train_size + test_size])
...
svr_lin = SVR(kernel='linear', C=1e1, max_iter=100)
#svr_poly = SVR(kernel='poly', C=1e3, degree=2)
#svr_rbf = SVR(kernel='rbf', C=1e3, gamma=0.1)

print("start training")
svr_lin.fit(dataset_X[0 : train_size], dataset_Y[0 : train_size])
print("done training")

from sklearn.externals import joblib
joblib.dump(svr_lin, 'linear_regression_model.pkl')
'''
